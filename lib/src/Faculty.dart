...
import 'Widget/loginscreen/Faculty_Home.dart';
...
class _FacultyState extends State<Faculty> {
  ...
  void logIn() {
    FirebaseAuth.instance.signInWithEmailAndPassword(
      ...
      setState(() {
        ...
        _teddyController.setres(res);
        _teddyController.submitPassword();
        firestoreInstance.collection("Faculty").doc(_emailController.substring(0,16)).get().then((value){
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => FHomeScreen(signOut),),
  ...
  Widget _emailbutton() {
    return InkWell(
      onTap: () {
        Navigator.of(context).pop();
        if(_email.text.contains("@cb.faculties.amrita.edu")){
          forgot();
        }
        else{
          showDialog(
              ...
              builder: (BuildContext context) {
                return AlertDialog(
                  ...
                  content: Text("You are not authorised to change password in faculty page!!!"),
                  actions: [
                    FlatButton(
                      child: Text("Ok"),
                      onPressed: () {
                                          ...
                                          SigninButton(
                                              ...
                                              onPressed: () {
                                                if (_formKey.currentState.validate()) {
                                                  if(_emailController.contains("@cb.faculties.amrita.edu")){
                                                    setState(() {
                                                    ...
                                                    showDialog(
                                                        ...
                                                        builder: (BuildContext context) {
                                                          return AlertDialog(
                                                            title: Text("NOT AUTHORISED"),
                                                            content: Text("You are not authorised to login through faculty page!!!"),
                                                            actions: [
                                                              FlatButton(
                                                                child: Text("Ok"),
                                                                onPressed: () {
